...
from .config import (global_args, update_uvicorn_mode_config, get_default_host,)
...
# print(f"EMBEDDING_BINDING: {os.getenv('EMBEDDING_BINDING')}")  
# print(f"EMBEDDING_BINDING_HOST: {os.getenv('EMBEDDING_BINDING_HOST')}")
...
def create_app(args):
    ...
    # api_version_display = __api_version__
    ...
    # 是在创建一个文档管理器实例，负责处理文档的输入目录管理和工作空间隔离
    # 这个设计使得 LightRAG 可以同时为多个用户或项目提供服务，而各方的数据完全隔离。
    # https://deepwiki.com/search/displaysplashscreen_f2969a66-afd6-40ee-a66b-05434aafc6ad?mode=fast#4
    doc_manager = DocumentManager(args.input_dir, workspace=args.workspace)
    # doc_manager = DocumentManager(args.input_dir, workspace="") # 简化工作空间
    ...
    # swagger_description = (base_description + (" (API-Key Enabled)" if api_key else "") + "\n\n[View ReDoc documentation](/redoc)")
    # app_kwargs = {
    ...
    #     "lifespan": lifespan,
    # }
    # # Configure Swagger UI parameters； Enable persistAuthorization and tryItOutEnabled for better user experience
    # app_kwargs["swagger_ui_parameters"] = {
    #     "persistAuthorization": True,
    #     "tryItOutEnabled": True,
    # }
    # app = FastAPI(**app_kwargs)
    app = FastAPI(title="LightRAG Backend",  lifespan=lifespan,) # 最小化 APP 配置; 仅保留生命周期管理  
    ...
    def create_llm_model_func(binding: str):
        """ Create LLM model function based on binding type. Uses optimized functions for OpenAI bindings and lazy import for others. """
        try:
            if binding == "lollms":
    ...
    #     """ Create LLM model kwargs based on binding type. Uses lazy import for binding-specific options. """
    ...
    def create_optimized_embedding_function(config_cache: LLMConfigCache, binding, model, host, api_key, args) -> EmbeddingFunc:
        """ Create optimized embedding function and return an EmbeddingFunc instance with proper max_token_size inheritance from provider defaults.
        This function:
        ...
        try:
            ...
            if provider_func and isinstance(provider_func, EmbeddingFunc):
                ...
                logger.debug(f"Extracted from {binding} provider: max_token_size={provider_max_token_size}, embedding_dim={provider_embedding_dim}")
        except ImportError as e:
        ...
        async def optimized_embedding_function(texts, embedding_dim=None):
            try:
                if binding == "lollms":
                    ...
                    return await actual_func(texts, embed_model=model, host=host, api_key=api_key)
                # elif binding == "ollama":
                #     from lightrag.llm.ollama import ollama_embed
                #     # Get real function, skip EmbeddingFunc wrapper if present
        ...
        logger.info(f"Embedding config: binding={binding} model={model} embedding_dim={final_embedding_dim} max_token_size={final_max_token_size}")
    ...
    logger.info(
        ...
        f"(dimensions={embedding_func.embedding_dim}, has_param={has_embedding_dim_param}, binding={args.embedding_binding})"
    )
    ...
    #     async def server_rerank_func(query: str, documents: list, top_n: int = None, extra_body: dict = None):
    ...
    #     logger.info(f"Reranking is enabled: {args.rerank_model or 'default model'} using {args.rerank_binding} provider")
    # else:
...
def main():
    ...
    # from multiprocessing import freeze_support # 为 Windows 多进程环境提供支持
    # freeze_support()

    # Configure logging before parsing args
    configure_logging()
    update_uvicorn_mode_config() # 和多进程有关， 用户可能在 .env 中设置 WORKERS=4，但使用 lightrag-server 启动； 此函数会强制将 workers 改为 1，避免配置冲突
    ...
    # create_app() 函数创建并配置了完整的后端服务，包括所有路由、存储、LLM 连接
    app = create_app(global_args)

    # Start Uvicorn in single process mode   -   单进程模式 - 使用 uvicorn.run() 启动单进程
    # 配置 Uvicorn 参数（主机、端口、SSL 等）； 启动单进程模式的 Uvicorn 服务器
    #   lightrag-server
    # lightrag/api/run_with_gunicorn.py   使用 Gunicorn 管理多个 worker 进程： code： # Force workers to be an integer and greater than 1 for multi-process mode
